...
$(document).ready(function () {
  ...
  $(".dw-add-to-saved").on('click', function (e) {
    e.preventDefault();
    var thisObj = $(this);
    var heartIcons = $(".dw-add-to-saved").find("i.fa-heart");
    var planId = thisObj.data('plan-id');
    $.ajax({
      method: 'POST',
      url: "/save-plan/".concat(planId),
      headers: {
      ...
      if (data.status == "add") {
        heartIcons.each(function (index, element) {
  ...
  $("#dw-promo-email").on('keypress', function (e) {
    var email = $("#dw-promo-email").val();

    if (e.keyCode == 13) {
      dwPromoSend(email);
    }
  });
  $("#dw-promo-submit").on('click', function (e) {
    e.preventDefault();
    var email = $("#dw-promo-email").val();
    dwPromoSend(email);
  });
...
function dwPromoSend(email) {
  $.ajax({
    method: 'POST',
    url: "/promo-email-send/".concat(email),
    headers: {
      'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    },
    dataType: "json"
...
__webpack_require__(/*! /Users/stasdre/server/dwhp/resources/assets/js/app.js */"./resources/assets/js/app.js");
__webpack_require__(/*! /Users/stasdre/server/dwhp/resources/assets/sass/main.scss */"./resources/assets/sass/main.scss");
module.exports = __webpack_require__(/*! /Users/stasdre/server/dwhp/resources/assets/sass/admin/app.scss */"./resources/assets/sass/admin/app.scss");
