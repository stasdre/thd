  ...
  <div class="row no-gutters align-items-center py-2 px-2">
    ...
    <div class="col-4">
      <ul class="list-inline mb-0 text-right">
        <li class="list-inline-item"><a href=""><i class="fa fa-heart-o" style="color:white;font-size : 25px;"></i></a>
        </li>
      </ul>
    </div>
  </div>
  ...
  <div class="row no-gutters">
    ...
    <div class="col-10 xs_portrait_width_100">
      <div class="slider">
        ...
        <figure> <img src="{{ asset('storage/plans/'.$plan->id.'/'.$image->file_name) }}" alt="{{$image->alt_text}}"
            class="img-fluid">
    ...
    <div class="col xs-hide-portrait">
      <ul class="list-unstyled single-plan-actions text-center">
        ...
        <li><a href="#" class="text-secondary"><img src="{{asset('images/icons/icon-print.png')}}" alt=""
              class="img-fluid"></a></li>
      ...
      <div class="py-2 px-5 font-weight-bold">
        <div class="row text-center no-gutters">
          <div class="col py-1 border border-light font-weight-semi-bold">Sq. Ft. <span
              class="d-block text-secondary"><b>{{$plan->square_ft['str_total']}}</b></span></div>
          <div class="col py-1 border border-light font-weight-semi-bold bg-secondary">1st Fl <span
              class="d-block text-white"><b>{{$plan->square_ft['1_floor']}}</b></span></div>
          <div class="col py-1 border border-light font-weight-semi-bold">2nd Fl <span
              class="d-block text-secondary"><b>{{$plan->square_ft['2_floor']}}</b></span></div>
          <div class="col py-1 border border-light font-weight-semi-bold bg-secondary">Garages <span
              class="d-block text-white"><b>{{$plan->garage['car']}} car</b></span></div>
        </div>

        <div class="row text-center no-gutters">
          <div class="col py-1 border border-light font-weight-semi-bold bg-secondary">Beds <span
              class="d-block text-white"><b>{{$plan->rooms['r_bedrooms']}}</b></span></div>
          <div class="col py-1 border border-light font-weight-semi-bold">Baths <span
              class="d-block text-secondary"><b>{{$plan->rooms['r_full_baths']}}</b></span></div>
          <div class="col py-1 border border-light font-weight-semi-bold bg-secondary">Width <span
              class="d-block text-white"><b>{{$plan->dimensions['width_ft']}}'
                {{$plan->dimensions['width_in']}}"</b></span></div>
          <div class="col py-1 border border-light font-weight-semi-bold">Depth <span
              class="d-block text-secondary"><b>{{$plan->dimensions['depth_ft']}}'
                {{$plan->dimensions['depth_in']}}"</b></span></div>
        </div>
      </div>
      <a href="#floorPlan" class="btn btn-primary text-uppercase rounded-0 mt-3 xs-hide-portrait">View Floor Plans</a>
    </div>
          ...
          <div class="form-group mb-0">
            <div class="select-custom-wrap select-custom-wrap-lg">
              <select name="plan_package" class="select-custom white_back">
                ...
                <option value="{{$package->id}}" data-cost="{{$package->pivot->price }}">{{$package->name}}
                  ({{$package->pivot->price }}$)</option>
                @endforeach
              </select>
            </div>
          </div>
          ...
          <div class="form-group mb-0">
            <div class="select-custom-wrap select-custom-wrap-lg">
              <select name="plan_foundation" class="select-custom white_back">
                ...
                <option value="{{$foundat->id}}" data-cost="{{$foundat->pivot->price}}">{{$foundat->name}}
                  ({{$foundat->pivot->price}}$)</option>
                @endforeach
              </select>
            </div>
          </div>
          ...
          <div class="form-group mb-0">
            <div class="select-custom-wrap select-custom-wrap-lg">
              <select name="plan_features" class="select-custom white_back">
                ...
                <option value="{{$addon->id}}" data-cost="{{$addon->pivot->price}}">{{$addon->name}}
                  ({{$addon->pivot->price}}$)</option>
                @endforeach
              </select>
            </div>
          </div>
...
<div class="tab-content bg-secondary px-5 py-3 mt-2" id="myTabContent" style="margin-top : 0 !important;">
  <div class="tab-pane fade show active" id="planDescription" role="tabpanel">
    ...
    @if($plan->short_desc)
    <div class="margin_bt0-xs">{!! $plan->short_desc !!} <a href="#" data-toggle="collapse" data-target="#readMore"
        class="read_more"> READ MORE...</a>. <div id="readMore" class="collapse in">{!! $plan->description !!}</div>
    </div>
    @else
...
<div class="">
  <ul class="nav nav-pills plan-nav nav-justified text-uppercase mt-2" id="planDetails" role="tablist">
    ...
    <li class="nav-item border-right-xs"> <a class="nav-link rounded-0" id="modifyplan" target="_blank"
        href="{{route('modify-plan', $plan->plan_number)}}">Modify <span class="xs-hide-portrait"> This Plan</span></a>
    </li>
    <li class="nav-item border-left-xs"> <a class="nav-link rounded-0" id="costbuild" data-toggle="tab"
    ...
    <li class="nav-item xs-hide-portrait"> <a class="nav-link rounded-0" id="customerreviews" data-toggle="tab"
        href="#customerReviews" role="tab">Customer Reviews</a> </li>
  </ul>
  @if ($agent->isDesktop() || $agent->isTablet())
    ...
    <div class="tab-pane fade" id="planSpecifications" role="tabpanel">
      <div class="row">
        ...
        <div class="col-sm-6 col-lg-4">
          ...
          <p><span class="left_boldTxt" style="color: transparent;">..</span><span class="text_paddindLeft">Optional:
              Crawlspace, Basement</span></p>
          <p><span class="left_boldTxt"><b>Ceiling Height</b></span><span class="text_paddindLeft">1st floor: 10’; 2nd
              floor: 9’</span></p><br>
    ...
    <div class="tab-pane fade show active" id="first" role="tabpanel">
      ...
      <div class="col-lg-10 m-auto img-container-reverse">
        ...
        <div class="row mobile-off">
          <div class="col-md-3 text-xs-center"><a href="#purchase" role="button"
              class="btn btn-primary text-uppercase rounded-0">PURCHASE PLANS</a></div>
          <div class="col-md-6 text-center">
            <a download target="_blank" href="{{asset('storage/plans/'.$plan->id.'/'.$image->file_name)}}"
...
<div class="text-center px-5">
  <p><a href="#" class="btn btn-primary rounded-0 mt-3" data-toggle="collapse" data-target="#ViewallPlans">View ALL plan
      details</a></p>
</div>
<div class="row only_under_767">
  <div class="col-md-12">
    <div class="text-center text-uppercase lead font-weight-normal align-middle py-2" style="font-size: 19px;"> <a
        href="#" class="align-middle text-primary">Live Chat</a> | <a href="#"
        class="align-middle text-primary">Email</a> | <a href="#" class="align-middle text-secondary">xxx-xxx-xxxx</a>
    </div>
  </div>
</div>
...
<div class="col-lg-2 col-sm-5 padd_top pd-md-0 center plan-information">
  <!--d-sm-none-->
  <div class="bg-secondary px-2 py-2 mb-1 ">
    <h5 class="text-primary">Get Your Plans in Minutes</h5>
    <!--<p class="small m-0">The PDF Plan Package offers Immediate delivery of your House Plans!</p>-->
...
<div class="row text-center">
  <div class="col-md-6">
    ...
    <div class="row text-center">
      <div class="col-sm-6">
        <div class="plan-grid"> <a href="#"> <img src="{{asset('images/plan-1.jpg')}}" alt="New House Plans"
              class="img-fluid" />
